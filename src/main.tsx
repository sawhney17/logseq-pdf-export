...
export function downloadPDF(isBullets: boolean) {
  console.log(isBullets);
  console.log(isBullets);
  console.log(isBullets);
  if (isBullets) {
    window.print();
...
function renderApp(env: string, isBullets: boolean) {
  let head = top.document.head.innerHTML;
  let env1 = top.document.body.innerHTML;

  let env2 = "<html>" + head + env1 + "</html>";
  console.log(isBullets);
  if (isBullets) {
    ReactDOM.render(
      <React.StrictMode>
        <App3 htmlText={env} isBullets={isBullets} />
      </React.StrictMode>,
    ...
    ReactDOM.render(
      <React.StrictMode>
        <App htmlText={env} isBullets={isBullets} />
      </React.StrictMode>,
      document.getElementById("root")
    );
  }
  ...
  logseq.App.getCurrentGraph().then(async (graph) => {
    ...
    if (logseq.settings[templateName + "Options"].includes("Hide Bullets")) {
      ...
      let finalResult = (
        ...
        .replaceAll("</p>", "</span></p>");
      renderApp(finalResult, true);
    } else {
      final4String = final4String
        .replaceAll("<p>", "<p><span>• ")
        .replaceAll("</p>", "</span></p>");
      renderApp(final4String, false);
    }
    handleClosePopup();
  });
